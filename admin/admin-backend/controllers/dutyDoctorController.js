...
const addDutyDoctor=async(req,res)=>{
    try {
        ...
        res.status(201).json({ message: 'DutyDoctor added successfully', dutydoctor: newDutyDoctor });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Failed to add dutydoctor', error: error.message });
    }
}
